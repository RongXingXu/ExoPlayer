 ...
 *
 * <p>ExoPlayer is designed to make few assumptions about (and hence impose few restrictions on)
 * the type of the media being played, how and where it is stored, and how it is rendered. Rather
 * than implementing the loading and rendering of media directly, ExoPlayer implementations delegate
 * this work to components that are injected when a player is created or when it's prepared for
 * playback. Components common to all ExoPlayer implementations are:
 *
 * <ul>
...
public interface ExoPlayer extends Player {
    ...
    final class Builder {
         ...
         *
         * <p>The strategy only applies if a {@link MediaCodec}-based video {@link Renderer} is
         * enabled. Applications wishing to use {@link Surface#CHANGE_FRAME_RATE_ALWAYS} should set the
         * mode to {@link C#VIDEO_CHANGE_FRAME_RATE_STRATEGY_OFF} to disable calls to {@link
     ...
     *
     * <p>For example, a video renderer will return {@link C#TRACK_TYPE_VIDEO}, an audio renderer
     * will return {@link C#TRACK_TYPE_AUDIO} and a text renderer will return {@link
     * C#TRACK_TYPE_TEXT}.
     *
     ...
     *                      position defined by {@link #getCurrentMediaItemIndex()} and {@link
     *                      #getCurrentPosition()}.
     */
     ...
     *                            C#TIME_UNSET} is passed, the default position of the given media
     *                            item is used. In any case, if {@code startMediaItemIndex} is set to
     *                            {@link C#INDEX_UNSET}, this parameter is ignored and the position is
     *                            not reset at all.
     */
     ...
     *                      false, playback will start from the position defined by {@link
     *                      #getCurrentMediaItemIndex()} and {@link #getCurrentPosition()}.
     */
    ...
    * */
     ...
     * scheduling and wait for {@link AudioOffloadListener#onExperimentalOffloadSchedulingEnabledChanged(boolean)}
     * to be called with {@code offloadSchedulingEnabled = false} before interacting with the player.
     *
     * <p>This mode should save significant power when the phone is playing offload audio with the
     * screen off.
     *
     * <p>This mode only has an effect when playing an audio track in offload mode, which requires
     * all the following:
     *
     * <ul>
